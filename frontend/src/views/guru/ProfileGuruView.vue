  ...
  <div class="dashboard-wrapper">
    <aside class="sidebar">
      ...
      <nav class="nav-menu">
        ...
        <router-link to="/guru/profile" class="nav-item active"><i class="bi bi-person-fill"></i><span>Profile</span></router-link>
      </nav>
    ...
    <main class="main-content">
      <header class="topbar">
        <div>
          <h1>Profil Saya</h1>
          <p>Kelola data akun dan informasi pribadi Guru.</p>
        </div>
      </header>

      <section class="card-box">
        <form @submit.prevent="simpanProfile">
          <div class="form-group"><label>NIP / NUPTK</label><input v-model="profile.nip" class="form-input" /></div>
          <div class="form-group"><label>Nama Lengkap</label><input v-model="profile.nama" class="form-input" /></div>
          <div class="form-group"><label>Email</label><input v-model="profile.email" type="email" class="form-input" /></div>
          <div class="form-group"><label>No. WhatsApp</label><input v-model="profile.noHp" class="form-input" /></div>
          <button type="submit" class="btn-add">Perbarui Profil</button>
        </form>
      </section>
    </main>
  </div>
...
const profile = reactive({
  ...
  nama: 'Guru Pembimbing, S.Kom',
  email: 'guru.walikelas@sekolah.sch.id',
  noHp: '081299887766'
})

const simpanProfile = () => { alert('Profil berhasil disimpan!') }
