  ...
  <div class="dashboard-wrapper">
    <aside class="sidebar">
      ...
      <nav class="nav-menu">
        ...
        <router-link to="/guru/transaksi" class="nav-item active"><i class="bi bi-receipt"></i><span>Transaksi</span></router-link>
    ...
    <main class="main-content">
      <header class="topbar">
        <div>
          <h1>Transaksi Kas Kelas</h1>
          <p>Persetujuan pembayaran iuran & Input pembayaran tunai (Cash).</p>
        </div>
        <button class="btn-add" @click="showCashModal = true">+ Input Bayar Cash</button>
      </header>
      ...
      <section class="table-section">
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="t in daftarTransaksi" :key="t.id">
              <td>#{{ t.id }}</td>
              <td><strong>{{ t.siswa }}</strong></td>
              <td>Rp {{ t.nominal.toLocaleString('id-ID') }}</td>
              <td>{{ t.metode }}</td>
              <td>
                <span :class="['badge', getBadge(t.status)]">{{ t.status }}</span>
              </td>
              <td>
                <div v-if="t.status === 'Pending'" class="action-buttons">
                  <button class="btn-approve" @click="updateStatus(t.id, 'Disetujui')">Approve</button>
                  <button class="btn-delete" @click="updateStatus(t.id, 'Ditolak')">Reject</button>
                </div>
                <span v-else class="text-muted">Selesai</span>
              </td>
            </tr>
          </tbody>
        </table>
...
const daftarTransaksi = ref([
  { id: 101, siswa: 'Budi Santoso', nominal: 20000, metode: 'Transfer/Midtrans', status: 'Pending' },
  { id: 102, siswa: 'Ani Rahayu', nominal: 10000, metode: 'Cash (Tunai)', status: 'Disetujui' }
])

const updateStatus = (id, status) => {
  const item = daftarTransaksi.value.find(t => t.id === id)
